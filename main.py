...
from kivy.utils import platform  # pyright: ignore[reportMissingImports]
...
class IntervalometerRoot(FloatLayout):
    ...
    total_text = StringProperty("TOTAL 00:16")
    running = False
    shot_count = 0
    elapsed = 0.0
    ...
    def _update_total(self):
        total = self.settings.delay + self.settings.number * self.settings.exposure + max(0, self.settings.number - 1) * self.settings.interval
    ...
    def _release_shot(self, *_):
        if self.running and self.shot_count < self.settings.number:
            ...
            self._shot_event = Clock.schedule_once(self._trigger_shot, self.settings.interval)
        elif self.running:
            self.running = False
            self.crosshair.end_hold()
            self._clear_highlights()
    ...
    def on_start(self):
        ...
        try:
            ...
            if not settings.canDrawOverlays(activity):
                intent = autoclass("android.content.Intent")("android.settings.action.MANAGE_OVERLAY_PERMISSION")
                uri = autoclass("android.net.Uri").parse("package:" + activity.getPackageName())
                intent.setData(uri)
                activity.startActivity(intent)
                return
            autoclass("org.intervalometer.OverlayService").start(activity)
            Window.hide()
        except Exception:
            pass
